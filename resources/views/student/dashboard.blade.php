...
@section('content')
    @if(!$student)
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-8">
            <h3 class="text-xl font-bold text-yellow-900 mb-3">Student Profile Not Complete</h3>
            <p class="text-yellow-700 mb-6">Please complete your student profile to access all dashboard features.</p>
            <a href="{{ route('student.profile') }}" class="inline-block bg-primary hover:bg-primary-hover text-white px-8 py-3 rounded-full font-medium transition-all btn-press">
                Complete Profile
            </a>
        </div>
    @else
        <!-- Welcome Section -->
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-neutral-900 mb-2">Welcome back, {{ auth()->user()->name }}!</h2>
            <p class="text-neutral-500">Here's what's happening with your education funding.</p>
        </div>

        <!-- Quick Actions -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <a href="{{ route('student.funding-requests.create') }}" class="bg-surface p-4 rounded-xl border border-neutral-200 text-center hover:bg-neutral-50 transition-colors">
                <div class="w-12 h-12 bg-primary-soft rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                </div>
                <p class="font-medium text-neutral-900">New Request</p>
            </a>
            <a href="{{ route('student.documents') }}" class="bg-surface p-4 rounded-xl border border-neutral-200 text-center hover:bg-neutral-50 transition-colors">
                <div class="w-12 h-12 bg-primary-soft rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                </div>
                <p class="font-medium text-neutral-900">Documents</p>
            </a>
            <a href="{{ route('student.milestones.index') }}" class="bg-surface p-4 rounded-xl border border-neutral-200 text-center hover:bg-neutral-50 transition-colors">
                <div class="w-12 h-12 bg-primary-soft rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                    </svg>
                </div>
                <p class="font-medium text-neutral-900">Milestones</p>
            </a>
            <a href="{{ route('student.wallet') }}" class="bg-surface p-4 rounded-xl border border-neutral-200 text-center hover:bg-neutral-50 transition-colors">
                <div class="w-12 h-12 bg-primary-soft rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                </div>
                <p class="font-medium text-neutral-900">Wallet</p>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-medium bg-green-100 text-green-800 px-2 py-1 rounded-full">
                            +{{ $stats['total_funding_requests'] ?? 0 }}
                        </span>
                    </div>
                </div>
                <p class="text-sm text-neutral-500 mb-1">Total Requests</p>
            ...
            <div class="bg-surface p-6 rounded-xl border border-neutral-200 hover-lift">
                ...
                <p class="text-sm text-neutral-500 mb-1">Wallet Balance</p>
                <p class="text-3xl font-bold text-primary">${{ number_format($walletBalance ?? 0, 2) }}</p>
            </div>
        </div>
        ...
        <!-- Recent Funding Requests and Activities -->
                ...
                <div class="p-6">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-neutral-500 mb-4">No funding requests yet</p>
                            <a href="{{ route('student.funding-requests.create') }}" class="inline-block bg-primary hover:bg-primary-hover text-white px-6 py-2 rounded-full text-sm font-medium transition-all btn-press">
